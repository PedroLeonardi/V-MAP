...
export default function ModalRelatorioAdm({ isVisible, onClose }) {
  ...
  useEffect(() => {
    if (isVisible) {
      axios.get('http://localhost:3001/rotas')
        .then((response) => {
  ...
  const rotasFiltradas = rota.filter((rota) =>
      ...
      <div className="w-full max-w-md bg-gradient-to-br from-gray-900 via-gray-800 to-gray-700 p-6 sm:p-8 rounded-xl shadow-2xl border border-gray-700 relative transform transition-all duration-300 scale-100 hover:scale-[1.01] max-h-[90vh] overflow-y-auto">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm text-left text-white border border-gray-600">
            ...
            <tbody>
              {/* eu passo a const rotasFiltradas */}
              {rotasFiltradas.map((rota) => (
                <tr key={rota.rota_id} className="hover:bg-gray-700">
                  <td className="px-4 py-2 border border-gray-600">{rota.rota_id}</td>
                  <td className="px-4 py-2 border border-gray-600">{rota.coordernadas}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {rotasFiltradas.length === 0 && (
          <p className="text-center text-gray-400 mt-4">Nenhum rota encontrado.</p>
        )}
      </div>
    </div>
  );
