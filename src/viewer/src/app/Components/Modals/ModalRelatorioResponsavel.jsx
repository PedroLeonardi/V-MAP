...
export default function ModalRelatorioResponsaveis({ isVisible, onClose }) {
  const [responsaveis, setResponsaveis] = useState([])
  const [busca, setBusca] = useState('') // sistema de busca

  // Requisição para a API quando o modal abrir
  useEffect(() => {
    if (isVisible) {
      axios.get('http://localhost:3001/responsavel')
        .then((response) => {
  ...
  // Lógica do filtro (ignora maiúsculas/minúsculas)
  ...
  return (
    ...
    >
      <div className="w-full max-w-md bg-gradient-to-br from-gray-900 via-gray-800 to-gray-700 p-6 sm:p-8 rounded-xl shadow-2xl border border-gray-700 relative transform transition-all duration-300 scale-100 hover:scale-[1.01] max-h-[90vh] overflow-y-auto">
        ...
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-white mb-1">Lista de Responsáveis Cadastrados</h2>
          ...
          <table className="min-w-full text-sm text-left text-white border border-gray-600">
            ...
            <tbody>
              {responsaveisFiltrados.map((responsavel) => (
                <tr key={responsavel.id_responsavel} className="hover:bg-gray-700">
                  <td className="px-4 py-2 border border-gray-600">{responsavel.id_responsavel}</td>
                  <td className="px-4 py-2 border border-gray-600">{responsavel.nome}</td>
                  <td className="px-4 py-2 border border-gray-600">{responsavel.cpf_responsavel}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {responsaveisFiltrados.length === 0 && (
          <p className="text-center text-gray-400 mt-4">Nenhum responsável encontrado.</p>
        )}
      </div>
    </div>
  );
